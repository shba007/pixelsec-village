...
onTick((delta) => {
    if (props.animation && tram.animation === 'started') {
        ...
        if (progress == 1) {
            ...
            if (currentTramPositionIndex.value < props.steps.length - 1)
...
<template>
    <AnimatedSprite :playing="animation && tram.animation === 'started'" :animation-speed="0.08"
        :textures="tram.aliases" :anchor="0.5" :x="tram.position.x" :y="tram.position.y" :scale="tram.position.scale" />
</template>
