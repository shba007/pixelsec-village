...
function onStart() {
  ...
  if (!isPressed.value) {
    ...
    if (!isSoundLoaded.value || !isSoundPlayed.value) return
...
<template>
  <main class="relative bg-black" :class="{ 'overflow-hidden portrait:h-[130vh]': isMobile.apple.phone && isStarted && (currentScreenIndex === 0 || gamePause) }" @click="onStart">
    <section>
      <img
        v-if="
    ...
    <Application class="relative z-10" :width="screenWidth" :height="screenHeight" :resolution="2" :antialias="motionBlur" power-preference="high-performance">
