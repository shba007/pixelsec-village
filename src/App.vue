...
const { currentScreenIndex, gamePause, motionBlur, isSoundLoaded, isStarted, isPressed } = storeToRefs(gameStore)
...
<template>
  <main class="relative"
    :class="{ 'overflow-hidden portrait:h-[130vh]': isMobile.apple.phone && isStarted && (currentScreenIndex === 0 || gamePause) }"
    @click="onClick">
      ...
      <Loader :resources="{ ...resources.font, ...resources.image }" :on-resolved="onResolve">
        ...
        <template #default>
          <template v-if="!isStarted">
            ...
            <Text v-else :x="loadingText.x" :y="loadingText.y" :anchor="0.5" :style="loadingText.style">Loading...
              99%</Text>
          </template>
          <template v-else>
